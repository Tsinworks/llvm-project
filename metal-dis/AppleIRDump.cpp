...
static cl::opt<bool> DontPrint("disable-output",
                               cl::desc("Don't output the .txt file"),
                               cl::Hidden);

static cl::opt<bool> ShowAnnotations(
    "show-annotations",
...
extra) uint32_t container_version_major : 8; uint32_t container_version_rev : 4;
// lo uint32_t container_version_minor : 4; // hi
...
static_assert(sizeof(metallib_header_control) == 68, "invalid program info
length");
...
sizeof(uint64_t) + sizeof(metallib_header_control), "invalid metallib header
size");
...
struct metallib_program_info {
        ...
        // NOTE: tag types are always followed by a uint16_t that specifies the
length of the tag data #define make_tag_type(a, b, c, d) ((uint32_t(d) << 24u) |
(uint32_t(c) << 16u) | (uint32_t(b) << 8u) | uint32_t(a)) enum TAG_TYPE :
uint32_t {
...
#undef make_tag_type
        ...
        struct offset_info {
                // NOTE: these are all relative offsets -> add to
metallib_header_control offsets to get absolute offsets uint64_t
reflection_offset; uint64_t debug_offset; uint64_t bitcode_offset;
        };
...
static Expected<bool> openInputFile(LLVMContext &Context,
                                    std::unique_ptr<ToolOutputFile> &Out) {
  auto &os = Out->os();

  //
  ErrorOr<std::unique_ptr<MemoryBuffer>> input_data =
  ...
  // sanity check
  if (buffer.size() < sizeof(metal::metallib_header)) {
    return make_error<StringError>("invalid header size",
                                   inconvertibleErrorCode());
  }

  //
  const auto &header = *(const metal::metallib_header *)data;
  ...
  // dump
  os << "[header]" << '\n';
  os << "container version: " << header.version.ver_lib.major << "."
     << header.version.ver_lib.minor << "." << header.version.ver_lib.rev << '\n';
  //os << "unknown version: " << header.version.unkown_version << "."
  //   << header.version.unknown_version_minor << "."
  //   << header.version.unknown_version_rev << '\n';
  ...
  for (uint32_t i = 0; i < header.header_control.program_count; ++i) {
    ...
    while (!found_end_tag) {
      const auto tag = *(const metal::LibraryMeta::Tag *)program_ptr;
      program_ptr += 4;
      uint32_t tag_length = 0;
      if (tag != metal::LibraryMeta::END) {
        tag_length = *(const uint16_t *)program_ptr;
        program_ptr += 2;

        if (tag_length == 0) {
          return make_error<StringError>("tag " + to_string(uint32_t(tag)) +
                                             " should not be empty",
                                         inconvertibleErrorCode());
        }
      }
      ...
      switch (tag) {
      case metal::LibraryMeta::NAME: {
        entry.name =
            llvm::StringRef((const char *)program_ptr, tag_length - 1u);
        break;
      }
      case metal::LibraryMeta::TYPE: {
        entry.type = *(const metal::LibraryMeta::FunctionType *)program_ptr;
        break;
      }
      case metal::LibraryMeta::HASH: {
        if (tag_length != 32) {
          return make_error<StringError>("invalid hash size: " +
                                             to_string(tag_length),
                                         inconvertibleErrorCode());
        }
        memcpy(&entry.sha256_hash, program_ptr, 32u);
        break;
      }
      case metal::LibraryMeta::OFFSET: {
        entry.offset = *(const metal::LibraryMeta::Offset *)program_ptr;
        break;
      }
      case metal::LibraryMeta::VERSION: {
        entry.api_version =
            *(const metal::LibraryMeta::VersionInfo *)program_ptr;
        entry.language_version =
            *((const metal::LibraryMeta::VersionInfo*)program_ptr + 1u);
        break;
      }
      case metal::LibraryMeta::MD_SIZE: {
        // TODO: this might not exist?
        entry.bitcode_size = *(const uint64_t *)program_ptr;
        break;
      }
      case metal::LibraryMeta::TESS: {
        entry.tess_info = *(const uint8_t *)program_ptr;
        break;
      }
      case metal::LibraryMeta::SOFF: {
        if (tag_length != 8) {
      ...
      case metal::LibraryMeta::END: {
        found_end_tag = true;
        break;
      }
      default:
        return make_error<StringError>("invalid tag: " +
                                           to_string((uint32_t)tag),
                                       inconvertibleErrorCode());
      }
  ...
  for (const auto &prog : info.entries) {
    ...
    os << "\ttype: ";
    switch (prog.type) {
    case metal::LibraryMeta::FunctionType::FRAGMENT:
      os << "fragment";
      break;
    case metal::LibraryMeta::FunctionType::VERTEX:
      os << "vertex";
      break;
    case metal::LibraryMeta::FunctionType::KERNEL:
      os << "kernel";
      break;
    case metal::LibraryMeta::FunctionType::NONE:
      os << "NONE";
      break;
    }
    os << '\n';
    ...
    auto bc_mem = WritableMemoryBuffer::getNewUninitMemBuffer(prog.bitcode_size,
                                                              "bc_module");
    ...
    os << "\tsize: " << bc_mem->getBufferSize() << ", " << prog.bitcode_size
       << '\n';
    os << '\n';
    memcpy((char *)bc_mem->getBufferStart(), data + bc_offset,
    ...
    if (bc_mod) {
      ...
      if ((*bc_mod)->materializeAll()) {
        return make_error<StringError>("failed to materialize",
                                       inconvertibleErrorCode());
      }
      (*bc_mod)->print(Out->os(), Annotator.get(),
...
int main(int argc, char **argv) {
  ...
  if (OutputFilename.empty()) { // Unspecified output, infer it.
    if (InputFilename == "-") {
      ...
      OutputFilename =
          (IFN.endswith(".metallib") ? IFN.drop_back(9) : IFN).str();
      OutputFilename += ".txt";
    }
  }
